...
const patches = [];
if (pending) { patches.push({ version: '1.0.' + build, date: today, title: esc(pending.subject), notes: pending.body.map(esc) }); build--; }
...
if (raw) for (const line of raw.split('\n')) {
  ...
  if (pending && s === pending.subject && patches.length === 1) continue; // avoid dup if already committed
  patches.push({ version: '1.0.' + build, date: d, title: esc(s), notes: [] });
  build--;
}
